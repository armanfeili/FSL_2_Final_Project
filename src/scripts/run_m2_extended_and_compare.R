...
cat("M2 extended MCMC (4ch, burnin=4000, iter=4000)...\n")
...
t2 <- system.time({
  ...
  yrep_ok <- TRUE
  tryCatch({
    s2_ext_yrep <- coda.samples(m2, "Y_rep", 4000)
    saveRDS(s2_ext_yrep, "src/outputs/model_objects/posterior_m2_extended_yrep.rds")
  }, error = function(e) {
    yrep_ok <<- FALSE
    writeLines(
      c(
        ...
        "note: posterior_m2_extended.rds saved before this error"
      ),
...
cat("M2 extended done in", round(t2[3], 1), "sec\n")
