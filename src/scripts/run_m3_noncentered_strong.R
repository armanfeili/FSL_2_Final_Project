...
log_step("M3 non-centered STRONG (4ch, adapt=1000, burnin=8000, iter=10000, monitors=globals+u+z, no Y_rep)")
...
t3 <- system.time({
  ...
  log_step("Stage 4/5: saving globals posterior (strong)")
  saveRDS(s3_glob, "src/outputs/model_objects/posterior_m3_noncentered_strong.rds")
  log_step("Stage 4/5: globals saved")

  log_step("Stage 5/5: sampling u and z (random effects)")
  s3_re <- coda.samples(m3, c("u", "z"), 10000, progress.bar = "none")
  saveRDS(s3_re, "src/outputs/model_objects/posterior_m3_noncentered_strong_u.rds")
  log_step("Stage 5/5: u+z saved")
})

writeLines(
  c(
    paste("timestamp:", as.character(Sys.time())),
    "model: M3 non-centered-strong",
    "stage: Y_rep sampling",
    "status: skipped",
    "note: Y_rep will be regenerated separately only if diagnostics pass"
  ),
  "src/outputs/diagnostics/m3_noncentered_strong_yrep_status.txt"
)

log_step(sprintf("M3 non-centered-strong fit complete in %.1f sec", t3[3]))

s <- readRDS("src/outputs/model_objects/posterior_m3_noncentered_strong.rds")
sm <- summary(s)
...
write.csv(tab, "src/outputs/diagnostics/m3_noncentered_strong_diagnostics_summary.csv", row.names = FALSE)
...
png("src/outputs/figures/m3_noncentered_strong_trace_density_core.png", width = 1400, height = 900, res = 140)
plot(s[, core])
dev.off()

png("src/outputs/figures/m3_noncentered_strong_autocorr_core.png", width = 1400, height = 900, res = 140)
...
comparison <- data.frame(
  metric = c("old_min_ess", "strong_min_ess", "old_max_rhat", "strong_max_rhat", "strong_all_pass"),
  value = c(old_min, nc_min, old_rmax, nc_rmax, nc_all)
)
write.csv(comparison, "src/outputs/diagnostics/m3_noncentered_strong_comparison.csv", row.names = FALSE)

log_step(sprintf("M3 strong summary: min ESS=%.1f  max Rhat=%.4f  all_pass=%s", nc_min, nc_rmax, nc_all))

if (nc_all) {
  file.copy(
    "src/outputs/model_objects/posterior_m3_noncentered_strong.rds",
    "src/outputs/model_objects/posterior_m3.rds",
    overwrite = TRUE
  )
  file.copy(
    "src/outputs/model_objects/posterior_m3_noncentered_strong_u.rds",
    "src/outputs/model_objects/posterior_m3_u.rds",
    overwrite = TRUE
  )
  file.copy(
    "src/outputs/diagnostics/m3_noncentered_strong_diagnostics_summary.csv",
    "src/outputs/diagnostics/m3_diagnostics_summary.csv",
    overwrite = TRUE
  )
  file.copy(
    "src/outputs/figures/m3_noncentered_strong_trace_density_core.png",
    "src/outputs/figures/m3_trace_density_core.png",
    overwrite = TRUE
  )
  file.copy(
    "src/outputs/figures/m3_noncentered_strong_autocorr_core.png",
    "src/outputs/figures/m3_autocorr_core.png",
    overwrite = TRUE
  )
  log_step("M3 standard files PROMOTED from non-centered-strong run (posterior, u, diagnostics, figures)")
  log_step("NEXT: old centered Y_rep is now invalid; regenerate posterior_m3_yrep.rds before Phase 10")
  file.remove("src/outputs/model_objects/posterior_m3_yrep.rds")
  log_step("Removed stale posterior_m3_yrep.rds to force Y_rep regeneration")
} else {
  log_step("M3 non-centered-strong kept as comparison only (NOT promoted). Posterior_m3.rds UNCHANGED.")
}
