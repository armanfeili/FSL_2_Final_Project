...
cat("M3 non-centered MCMC (4ch, burnin=4000, iter=4000, no Y_rep)...\n")
...
t3 <- system.time({
  ...
  saveRDS(s3_nc, "src/outputs/model_objects/posterior_m3_noncentered.rds")
})

writeLines(
  c(
    paste("timestamp:", as.character(Sys.time())),
    "model: M3 non-centered",
    "stage: Y_rep sampling",
    "status: skipped",
    "note: intentionally skipped to prioritize parameter diagnostics; previous M3 Y_rep remains available"
  ),
  "src/outputs/diagnostics/m3_noncentered_yrep_status.txt"
)

cat("M3 non-centered done in", round(t3[3], 1), "sec\n")

s <- readRDS("src/outputs/model_objects/posterior_m3_noncentered.rds")
sm <- summary(s)
...
write.csv(tab, "src/outputs/diagnostics/m3_noncentered_diagnostics_summary.csv", row.names = FALSE)
...
png("src/outputs/figures/m3_noncentered_trace_density_core.png", width = 1400, height = 900, res = 140)
plot(s[, core])
dev.off()

png("src/outputs/figures/m3_noncentered_autocorr_core.png", width = 1400, height = 900, res = 140)
...
write.csv(comparison, "src/outputs/diagnostics/m3_noncentered_comparison.csv", row.names = FALSE)

if (nc_all && (nc_min > old_min || nc_rmax <= old_rmax)) {
  file.copy(
    "src/outputs/model_objects/posterior_m3_noncentered.rds",
    "src/outputs/model_objects/posterior_m3.rds",
    overwrite = TRUE
  )

  file.copy(
    "src/outputs/diagnostics/m3_noncentered_diagnostics_summary.csv",
    "src/outputs/diagnostics/m3_diagnostics_summary.csv",
    overwrite = TRUE
  )

  if (file.exists("src/outputs/figures/m3_noncentered_trace_density_core.png")) {
    file.copy(
      "src/outputs/figures/m3_noncentered_trace_density_core.png",
      "src/outputs/figures/m3_trace_density_core.png",
      overwrite = TRUE
    )
  }

  if (file.exists("src/outputs/figures/m3_noncentered_autocorr_core.png")) {
    file.copy(
      "src/outputs/figures/m3_noncentered_autocorr_core.png",
      "src/outputs/figures/m3_autocorr_core.png",
      overwrite = TRUE
    )
  }

  message("M3 standard files promoted from non-centered run")
} else {
  message("M3 non-centered kept as comparison only (not promoted)")
}

message("M3nc min ESS: ", min(tab$ess[!tab$nonvarying], na.rm = TRUE),
