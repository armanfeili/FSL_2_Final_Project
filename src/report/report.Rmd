...
- Posterior predictive checks show M3 calibrates the mean (Bayesian p = 0.32) and substantially improves variance recovery, although the variance check remains near the lower tail (p = 0.04)
...
> **The hierarchical beta-binomial model (M3) is preferred.** It has the lowest DIC (24,940 vs 27,160 for M2 vs 2.6 million for M1) and best overall posterior predictive performance. M3 captures both overdispersion ($\phi = 42.9$) and persistent country-level heterogeneity ($\sigma_u = 0.72$).
...
*Figure: Average treatment success by WHO region and year. This complements the coverage heatmap by showing not only how many countries are retained in each region-year cell, but also how observed treatment success varies across regions and time.*
...
*Figure: Distribution of treatment success rates by WHO region. Each curve shows where country-year success rates are concentrated within a region. Most values are high, usually around 80-95%, but the curves are not identical across regions. Some regions also have a visible lower tail, meaning there are country-years with much lower success rates. This regional spread and lower-tail behavior motivate models that allow extra variability and country-level differences.*
...
*Figure: Mean treatment success rate over time by WHO region. Each panel shows the average success rate for one region in each year. The regional trends are not identical: some regions remain relatively stable, some improve, and some fluctuate more. These differences justify including WHO region in the models. Because countries within the same region can still differ substantially, the hierarchical model also adds country random effects.*
...
*Figure: Bivariate relationships among predictors and treatment success. These plots provide a visual check of how the main epidemiological predictors relate to the observed outcome before fitting the Bayesian models.*
...
*Figure: M1 trace and density plots for key parameters.*
...
*Figure: M2 trace and density plots for key parameters.*
...
*Figure: M3 trace and density plots for key parameters. The traces show stable, well-mixed chains with no visible trend or separation between chains, and the posterior densities overlap well across chains.*

Trace and density plots for the key parameters show stable mixing and overlapping posterior distributions across chains. Together with R-hat values below 1.05 and ESS values above 400 for the monitored key parameters, these diagnostics support using the posterior samples for inference.
...
*Figure: 95% credible interval coverage by parameter. The horizontal line marks 95%. Bars near or above the line indicate good calibration. All models show coverage close to the nominal level.*
...
> M3 is the preferred model. It has the lowest DIC and the best posterior predictive performance, though some residual miscalibration remains.
...
*Figure: M1 posterior predictive test statistics showing systematic misfit, especially for variance and lower-tail behavior.*
...
*Figure: M2 posterior predictive test statistics showing improved but still imperfect calibration.*
...
*Figure: M3 posterior predictive test statistics showing the best overall calibration among the three fitted models, although lower-tail and within-region miscalibration remain.*
...
*Figure: M1 cohort-stratified calibration. The binomial model shows poor calibration because it underestimates variability across cohort sizes.*
...
*Figure: M2 cohort-stratified calibration. The beta-binomial model improves calibration relative to M1 but still does not capture all structure in the data.*
...
*Figure: M3 calibration by cohort size. The hierarchical beta-binomial model provides the best cohort-level calibration among the three models, although some residual miscalibration remains.*
...
*Figure: M3 posterior predictive density overlay. The dark line is the observed distribution. Light lines are simulated datasets from the posterior.*
...
*Figure: Variance recovery across models. The vertical line marks the observed variance. M1's predictive distribution is far too narrow. M2 and M3 are much closer to the observed value, with M3 slightly better.*
...
*Figure: Country random effect caterpillar plot. Each horizontal line is a country's 95% credible interval. Countries above zero (right of center) have persistently higher success than expected. Countries below zero have persistently lower success. This is descriptive benchmarking, not a causal ranking.*
...
**The hierarchical beta-binomial model (M3) is preferred** for modeling country-year TB treatment success. It captures both overdispersion ($\phi = 42.9$) and persistent country heterogeneity ($\sigma_u = 0.72$), with the lowest DIC (24,940) and best posterior predictive performance.
