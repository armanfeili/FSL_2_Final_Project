...
(function() {
  var DEFAULT_ACTIVATION_THRESHOLD = 20; // px from top of viewport
  ...
  function initCustomScrollspy() {
    ...
    // Disable default tocify scroll behavior so custom logic is the single source of truth
    $(window).off('scroll.tocify');
    
    // Build a map of section IDs to TOC entries.
    // Support both anchor-based TOCs and li-only tocify items.
    var sectionMap = [];
    $headings.each(function() {
      ...
      if (sectionId) {
        var headingLabel = ($heading.text() || '').trim();
        var $tocLink = $toc.find('a[href="#' + sectionId + '"]').first();
        var $tocItem = $();

        if ($tocLink.length) {
          $tocItem = $tocLink.closest('li').first();
        } else {
          $tocItem = $toc.find('li.tocify-item').filter(function() {
            return ($(this).text() || '').trim() === headingLabel;
          }).first();
        }
    ...
    var activationThreshold = DEFAULT_ACTIVATION_THRESHOLD;
    if (window.getComputedStyle && sectionMap[0] && sectionMap[0].heading) {
      var marginTop = parseFloat(window.getComputedStyle(sectionMap[0].heading).scrollMarginTop || '0');
      if (!isNaN(marginTop) && marginTop >= 0) {
        activationThreshold = marginTop;
      }
    }
    
    function updateActiveSection() {
      var scrollTop = $(window).scrollTop();
      var viewportTop = scrollTop + activationThreshold;
      ...
      if (activeSection && activeSection.id !== currentActiveId) {
        ...
        if (activeSection.tocLink && activeSection.tocLink.length) {
